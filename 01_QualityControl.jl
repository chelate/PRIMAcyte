...
begin
    ...
    grouped = groupby(df_joined, :Image)
	# get illastik
    for group in grouped # iterate over each DataFrame group (i.e. image)
	...
	# identify cells to include
	df_joined.cell_included .= false
	...
	for df_image in groupby(df_joined,:Image)
		if sum(df_image.cell_included) > 2e3
...
plot(sort([sum(df_image.cell_included) for df_image in groupby(df_joined,:Image)]),
	range(0,1,length(groupby(df_joined,:Image))), xlabel = "# of cells", ylabel = "CDF", legend = false)
...
manifest_format = "2.0"
project_hash = "e86c4fcc86d64457b463a9b67877e335db8485c2"
